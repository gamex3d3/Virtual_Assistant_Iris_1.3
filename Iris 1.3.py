...
rate = machine.getProperty('rate')  # Get the current speech rate
machine.setProperty('rate', rate - 40)  
...
def input_instruction():
    try:
        with sr.Microphone() as origin:
            print_and_talk("Hi, How can I help?...")
            speech = listener.listen(origin, timeout=20)
...
def play_iris():
    while True:
        ...
        if "play" in instruction:
            ...
            query = instruction.replace("google", "")
            ...
            talk(f"Setting a timer for {timer_duration} seconds!")
            ...
            print_and_talk(f"Today's date is {current_date}")
            print_and_talk(f"The time right now is {current_time}")
            ...
            if "continue" in instruction:
                ...
                talk(f"I have found {len(news_res)} news articles. Let me tell you the first two of them.")
                talk(news_res[2])
                talk(news_res[3])
            ...
            talk("Alright, taking the screenshot")
